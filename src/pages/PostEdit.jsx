...
const LAYOUT_CLASSES = {
    title:
        "text-[var(--color-gray-8)] font-bold text-mo-title-xl tablet:text-tab-title-lg desktop:text-pc-title-lg",
    subtitle:
        "font-bold text-mo-title-md tablet:text-tab-title-md desktop:text-pc-title-md text-[var(--color-gray-5)]",
...
export default function PostEdit() {
    ...
    // 새로 추가한 파일: 훅으로 일원화 (PostCreate와 동일 패턴)
    // 총 허용 장수(MAX_IMAGES)에서 기존 서버 이미지를 제외한 남은 슬롯을 훅의 maxCount로 전달
    const remainingSlots = Math.max(0, MAX_IMAGES - keepImageNames.length);
    const {
    ...
    useEffect(() => {
        ...
        try {
            ...
            category: Array.isArray(rec.category)
                ? rec.category
                : rec.category
                ? JSON.parse(rec.category)
                : [],
            ...
            const names = Array.isArray(rec.images)
            ? rec.images
            : rec.images
            ? [rec.images]
            : [];
    ...
    const makeDescSubs = (text) => {
        ...
        return [{ text: "허용되지 않는 특수기호가 포함되어 있어요.", type: "error" }, ...subs];
        if (len > 1000)
        return [{ text: "최대 1000자까지 입력 가능해요.", type: "error" }, ...subs];
    ...
    const isNextEnabled = useCallback(() => {
        switch (step) {
        case 0:
            return isTitleValid(formData.title);
        case 1:
        ...
        case 4:
            return !!formData.timeStart && !!formData.timeEnd;
        case 5: {
    ...
    // 확인(step=7)에서 사용할 합쳐진 미리보기 (objectURL 정리 포함)
    const freshUrls = useMemo(
        () => editFiles.map((f) => URL.createObjectURL(f)),
        [editFiles]
    );
    const allPreviews = useMemo(
        () => [...existingImageUrls, ...freshUrls],
        [existingImageUrls, freshUrls]
    );
    useEffect(() => {
        return () => {
        // 생성된 blob URL 정리
        freshUrls.forEach((src) => {
        ...
        <>
            ...
            {showSkeleton ? (
                ...
                {step === 2 && (
                    ...
                    <div className={LAYOUT_CLASSES.titleAndInfoWrapper}>
                        ...
                        <SelectImageGroup
                            ...
                            onRemoveStatic={handleRemoveStatic} // 기존 서버 이미지 삭제(확인창 포함)
                        />
                    </div>
                    </div>
                )}
                ...
                {step === 6 && (
                    ...
                    <div className="relative">
                        ...
                        <Input
                            ...
                            "
                            subTexts={[{ text: "최소 2명, 최대 20명으로 모집 가능합니다.", type: "info" }]}
                        />
                        <b
                            className={[
                ...
                {step === 7 && (
                    ...
                    <ul
                        className={`${LAYOUT_CLASSES.titleAndInfoWrapper} p-3 bg-[var(--color-gray-2)] rounded-xl`}
                    >
                        ...
                        <div className={LAYOUT_CLASSES.InfoWrap}>
                            ...
                            <p key={item} className={TEXT_CLASSES.tag}>
                                {item}
                            </p>
                            ))}
                        </div>
                        ...
                        <div className={LAYOUT_CLASSES.InfoWrap}>
                            <p className={TEXT_CLASSES.timeTag}>
                            {formData.timeStartLabel || formData.timeStart}
                            </p>
                            <b className={TEXT_CLASSES.content}>부터</b>
                            <p className={TEXT_CLASSES.timeTag}>
                            {formData.timeEndLabel || formData.timeEnd}
                            </p>
